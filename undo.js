module.exports = function (bot, IO) {
	var undo = {
		...
		remove : function ( id, cb ) {
			...
			// FIXME, We have to allow the bot to work on several SE websites
			IO.request({
				url   : 'http://chat.stackoverflow.com/messages/' + id + '/delete',
				form: {
					fkey: bot.fkey
				},
				method  : 'POST',
		...
		update_id : function ( responseText ) {
			this.ids.push( JSON.parse(responseText).id );
		}
	};
	...
	bot.addCommand({
		...
		permissions : {
			del : 'NONE',
			use : 'OWNER' // CHANGE BACK TO OWNER!
		},
