...
var history = {
  ...
  handleResponse : function ( resp, params, cb ) {
    var self = this;
    jsdom.env(resp.parse.text['*'], ['http://code.jquery.com/jquery.js'], function(err, window) {

      var events = getEventsAsText( window.$('ul'), window );

      cb( self.filter(events, params) );
    });
  },
...
function getEventsAsText ( lists, window ) {
